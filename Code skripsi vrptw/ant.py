...
class Ant:
    ...
    def move_to_next_index(self, next_index):
        ...
        if self.graph.nodes[next_index].is_depot:
            ...
            # Update vehicle load, driving distance, time
    ...
    def check_condition(self, next_index) -> bool:
        ...
        # Check if a passenger can return to the service store after visiting the passenger
        ...
        # Cannot serve passengers outside of due time
    ...
    def cal_nearest_next_index(self, next_index_list):
        """
        Select the customer closest to the current position (ant.current_index) from the customers to be selected
    ...
    def try_insert_on_path(self, node_id, stop_event: Event):
        ...
        The insertion position cannot violate the load, time, and driving distance restrictions.
        If there are multiple locations, find the best one
        :param node_id:
        :return:
        """
        ...
        for insert_index in range(len(self.travel_path)):
            ...
            # Let check_ant walk through the points in the path starting from front_depot_index to insert_index-1
            for i in range(front_depot_index+1, insert_index):
                check_ant.move_to_next_index(self.travel_path[i])

            # Start trying to visit the nodes in the sorted index_to_visit
            if check_ant.check_condition(node_id):
                check_ant.move_to_next_index(node_id)
            else:
                continue

            # If the node_id can be reached, the vehicle must be able to drive back to the depot
            for next_ind in self.travel_path[insert_index:]:
               ...
               # If you cannot return to the depot, return to the previous level
                else:
                    break
    ...
    def insertion_procedure(self, stop_even: Event):
        ...
        # Until no node among the unvisited nodes can be successfully inserted
    ...
    def local_search_once(graph: VrptwGraph, travel_path: list, travel_distance: float, i_start, stop_event: Event):
        ...
        # Find the location of all depots in path
        depot_ind = []
        for ind in range(len(travel_path)):
            if graph.nodes[travel_path[ind]].is_depot:
                depot_ind.append(ind)

        # Divide self.travel_path into multiple segments, each segment starts and ends with a depot, called a route
                               ...
                               # Determine whether the changed route b is feasible
                                ...
                                if success_route_a and success_route_b:
                                    ...
                                    if new_path_distance < travel_distance:
                                        ...
                                        # Delete one of the depots in the path
